/*
 * @Author: jia-yf19
 * @Date: 2022-03-28 02:06:42
 * @LastEditTime: 2022-03-28 12:44:39
 * @Description: 通用工具类
 * @FilePath: /src/balance/src/utility.h
 */
...
template <typename T> T limit(const T &v, const T &lb, const T &ub) {
  if (v < lb) {
    return lb;
  } else if (v > ub) {
    return ub;
...
template <typename T> bool in_range(const T &v, const T &lb, const T &ub) {
  if (v < lb) {
    return false;
  } else if (v > ub) {
    return false;
