...
*/
void MotorSetting(uint16_t motor_id, uint8_t cmd, uint8_t *Data,
                  uint32_t *canID) {
  *canID = motor_id;
  if (cmd == 0)
    return;
...
void RV_can_data_repack(uint32_t msgID, uint8_t *Data, int32_t databufferlen,
  ...
  } else if (comm_mode == 0x00) // Response mode
  {
    ...
    if (ack_status == 1) // response frame 1
    {
      ...
      rv_motor_msg[motor_id_t].angle_actual_rad =
          uint_to_float(pos_int, POS_MIN, POS_MAX, 16);
      rv_motor_msg[motor_id_t].speed_actual_rad =
          uint_to_float(spd_int, SPD_MIN, SPD_MAX, 12);
      rv_motor_msg[motor_id_t].current_actual_float =
          uint_to_float(cur_int, I_MIN, I_MAX, 12);
    ...
    } else if (ack_status == 2) // response frame 2
    {
      ...
      rv_motor_msg[motor_id_t].current_actual_float =
          rv_motor_msg[motor_id_t].current_actual_int / 100.0f;
  ...
  } else if (comm_mode == 0x01) // automatic feedback mode
  {
    ...
    rv_motor_msg[motor_id_t].angle_actual_int =
        (uint16_t)(Data[0] << 8 | Data[1]);
    rv_motor_msg[motor_id_t].speed_actual_int =
