...
void arx_arm::get_curr_pos()
{
        ...
        current_pos[3] = rv_motor_msg[4].angle_actual_rad * (1 - filter_sensor) + current_pos[3] * filter_sensor;
        current_pos[4] = rv_motor_msg[5].angle_actual_rad * (1 - filter_sensor) + current_pos[4] * filter_sensor;
        current_pos[5] = rv_motor_msg[6].angle_actual_rad * (1 - filter_sensor) + current_pos[5] * filter_sensor;
...
void arx_arm::motor_control()
{
        ...
        // CAN_Handlej.Can_cmd_all(6, 5, 0.5, target_pos[4],target_vel[4], jointTorques(4));
        // CAN_Handlej.Can_cmd_all(7, 5, 0.5, target_pos[5], 0,jointTorques(5));

    
    ROS_INFO(">>>>>>>>>>>>>>>>> pos >>>>>>>>>>>>>>>>>>>");// 
    ROS_INFO("\033[32m pos = 1>%f 2>%f 3>%f 4>%f 5>%f 6>%f 7>%f \033[0m",  current_pos[0], \
                                                                                        current_pos[1], \
                                                                                        current_pos[2], \
                                                                                        current_pos[3], \
                                                                                        current_pos[4], \
...
//132
void arx_arm::can_position_control(uint16_t motor_id,float pos,uint16_t spd,uint16_t cur,uint8_t ack_status)
{
...
{   
    if(1){
        for(int i=0; i<7; i++)
        {
            ...
            ROS_INFO("\033[32mangle_actual_rad = %f %f %f %f %f %f %f \033[0m", rv_motor_msg[0].angle_actual_rad, \
                                                                                rv_motor_msg[1].angle_actual_rad, \
                                                                                rv_motor_msg[2].angle_actual_rad, \
                                                                                rv_motor_msg[3].angle_actual_rad, \
                                                                                rv_motor_msg[4].angle_actual_rad, \
                                                                                rv_motor_msg[5].angle_actual_rad, \
