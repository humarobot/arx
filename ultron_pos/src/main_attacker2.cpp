...
int main(int argc, char **argv) {
  ...
  Vector6d attack_pos[4];
  attack_pos[0] << 0.0, 1.0, 1.0, 0.0, 0.0, 0.0;
  attack_pos[1] << 1.0, 1.0, 1.0, 0.0, 0.0, 0.0;
  attack_pos[2] << -1.0, 1.0, 1.0, 0.0, 0.0, 0.0;
  attack_pos[3] << 0.0, 2.0, 2.0, 0.0, 0.0, 0.0;
  int switcher = 0;
  ...
  std::thread control_thread([&]() {
    while (ros::ok()) {
      {
        if (communicator.attack_rapid_) {
          q_cmd = attack_pos[switcher];
          switcher++;
          if (switcher > 3) switcher = 0;
          communicator.attack_rapid_ = false;
        }
        std::lock_guard<std::mutex> lock(qvt_mtx);
        q = q_cmd;
        v = v_cmd;
