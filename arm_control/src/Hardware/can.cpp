...
void can::GetImuGyro(uint8_t *data)
{
    ...
    // ImuMsg.angular_velocity.x = (int16_t)(data[0] << 8 | data[1]) * 50.0 / 32768;
    // ImuMsg.angular_velocity.y = (int16_t)(data[2] << 8 | data[3]) * 50.0 / 32768;
    // ImuMsg.angular_velocity.z = (int16_t)(data[4] << 8 | data[5]) * 50.0 / 32768;
    // ImuMsg.header.stamp = ros::Time::now();
}
...
void can::CAN_RMD_chassis(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
//    std::cout << "》》》》》》" << std::endl;
    can_frame_t frame;
    frame.can_id = 0x280;
