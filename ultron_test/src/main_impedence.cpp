...
pinocchio::SE3 oMdes(Eigen::Matrix3d::Identity(), Eigen::Vector3d(0.08, 0., 0.16));
pinocchio::SE3 oMdes_last(Eigen::Matrix3d::Identity(), Eigen::Vector3d(0.08, 0., 0.16));
...
void jointStateCallback(const sensor_msgs::JointState::ConstPtr &msg) {
  ...
  for (int i = 0; i < ultron_arm_now.joints.size(); i++) {
    ultron_arm_now.joints[i].acceleration =
        (ultron_arm_now.joints[i].velocity - ultron_arm_last.joints[i].velocity) / update_period;
    ultron_arm_now.joints[i].acceleration =
        0.05 * ultron_arm_now.joints[i].acceleration + 0.95 * ultron_arm_last.joints[i].acceleration;
  }
...
void poseCallback(const geometry_msgs::Pose::ConstPtr &msg) {
  ROS_INFO("End effector target position: [%f,%f,%f]", msg->position.x, msg->position.y, msg->position.z);
  ROS_INFO("End effector target orientation: [%f,%f,%f]", msg->orientation.x, msg->orientation.y, msg->orientation.z,
           msg->orientation.w);
  new_target = true;
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber joint_state_sub = nh.subscribe("ultron/joint_states", 10, jointStateCallback);
  ...
  ros::Publisher joint1_pub = nh.advertise<std_msgs::Float64>("/ultron/joint1_torque_controller/command", 1);
  ros::Publisher joint2_pub = nh.advertise<std_msgs::Float64>("/ultron/joint2_torque_controller/command", 1);
  ros::Publisher joint3_pub = nh.advertise<std_msgs::Float64>("/ultron/joint3_torque_controller/command", 1);
  ros::Publisher joint4_pub = nh.advertise<std_msgs::Float64>("/ultron/joint4_torque_controller/command", 1);
  ros::Publisher joint5_pub = nh.advertise<std_msgs::Float64>("/ultron/joint5_torque_controller/command", 1);
  ros::Publisher joint6_pub = nh.advertise<std_msgs::Float64>("/ultron/joint6_torque_controller/command", 1);
  // Init a joints acceleration topic publisher
  ros::Publisher joint_acceleration_pub =
      nh.advertise<std_msgs::Float64MultiArray>("ultron/joint_states_acceleration", 10);
  ...
  while (ros::ok()) {
    ...
    Eigen::VectorXd tau = data.nle - jac.transpose() * (Kp * err + Kd * (jac * v));
    ...
    for (int i = 0; i < 6; i++) {
      joint_acceleration_msg.data.push_back(ultron_arm_now.joints[i].acceleration);
    }
